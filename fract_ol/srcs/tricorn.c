...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/03 12:41:18 by euiclee           #+#    #+#             */
/*   Updated: 2022/11/07 12:49:26 by euiclee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_tricorn(t_data *data)
{
	...
	x = (data->x * ((data->map_right - data->map_left) / (double)WIN_X)
			+ data->map_left) + data->mv_x;
	y = (data->y * ((data->map_bottom - data->map_top) / (double)WIN_Y)
...
void	tricorn(void)
{
	...
	make_tricorn(&data);
	mlx_hook(data.win, KEY_PRESS, 0, tricorn_key, &data);
	mlx_hook(data.win, MOUSE_PRESS, 0, tricorn_mouse, &data);
